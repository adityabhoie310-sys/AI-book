import { BookOpen, FileText } from 'lucide-react';

const ViewChapterSidebar = ({ chapters = [], activeChapterId, onSelectChapter }) => {
  return (
    <aside className="w-full md:w-72 bg-white rounded-3xl border border-gray-100 p-5 shadow-xs">
      <h3 className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-4 flex items-center gap-2">
        <BookOpen className="w-4 h-4 text-purple-600" />
        Table of Contents
      </h3>
      <nav className="space-y-1.5">
        {chapters.map((chapter, index) => {
          const isActive = chapter._id === activeChapterId;
          return (
            <button
              key={chapter._id || index}
              onClick={() => onSelectChapter(chapter._id)}
              className={`w-full text-left p-3 rounded-2xl transition-all flex items-start gap-3 cursor-pointer ${
                isActive
                  ? 'bg-purple-50 text-purple-700 font-semibold shadow-xs border border-purple-100'
                  : 'text-gray-700 hover:bg-purple-50/50 hover:text-purple-700 font-medium'
              }`}
            >
              <span
                className={`w-5 h-5 rounded-full text-[11px] font-bold flex items-center justify-center shrink-0 mt-0.5 ${
                  isActive ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-500'
                }`}
              >
                {index + 1}
              </span>
              <div className="flex-1 min-w-0">
                <div className="text-xs truncate">{chapter.title}</div>
                <div className="text-[10px] text-gray-400 flex items-center gap-1 mt-0.5">
                  <FileText className="w-3 h-3" />
                  {chapter.wordCount || 0} words
                </div>
              </div>
            </button>
          );
        })}
      </nav>
    </aside>
  );
};

export default ViewChapterSidebar;
